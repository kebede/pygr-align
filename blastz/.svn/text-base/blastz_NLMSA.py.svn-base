...
class BlastzLocalAlignment:
    """
    A blastz gapped local alignment, consisting of multiple ungapped blocks.
    """
...
class BlastzUngappedBlock:
    """
    One ungapped block in a blastz alignment.
    """
...
def create_NLMSA_blastz(blastz_f, seqDb, al):
    """
        takes blastz output file as input and creates and returns NLMSA
    """
    
    blastzaln_list, genome_names = parse_blastz(open(blastz_f).read())
    ...
    for i in range(0,len(genome_names)):
        al += genomes_dict[genome_names[i]]
    ...
    for blz_al in blastzaln_list:
        ...
        for ungapped in block:
            ...
            ival1 = genomes_dict[genome_name1][a:b]
            ival2 = genomes_dict[genome_name2][x:y]
